...
// URL da API de Pagamento (backend local ou produção)
...
function exibirResultadoInicial(resultado) {
  ...
  container.innerHTML = `
    <div class="resultado-card resultado-inicial">
      <!-- Ícone de celebração ou atenção -->
...
function exibirResultadoAposCompleto() {
  ...
  container.innerHTML = `
    <div class="resultado-card resultado-final">
      ...
      <!-- Card Especialista (Centralizado) -->
...
async function processarPagamento(plano) {
  ...
  try {
    ...
    // Calcular valor com abatimento se for plano completo
    let valorPagamento = null;
    if (plano === 'completo' && resultadoState.planoBasicoPago) {
      // Aplica abatimento: valor completo - valor já pago
      valorPagamento = Math.round((PRECO_COMPLETO - PRECO_BASICO) * 100); // Em centavos
    }
    ...
    const response = await fetch(`${API_URL}/create-payment`, {
      ...
      body: JSON.stringify({
        nome: dadosFormulario.nome || 'Cliente',
        ...
        valorComAbatimento: valorPagamento // Valor em centavos com abatimento
      })
    });
    ...
    if (data.sucesso && data.pagamento) {
      ...
      resultadoState.paymentId = data.pagamento.id;
      
      // Abrir link de pagamento do Asaas em nova aba
      if (data.pagamento.invoiceUrl) {
        window.open(data.pagamento.invoiceUrl, '_blank');
      }
      
      // Fechar modal e mostrar área de confirmação
      fecharModalPagamento();
      mostrarAreaConfirmacaoPagamento(plano, data.pagamento.id);
